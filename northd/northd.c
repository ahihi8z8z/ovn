...
//+++ Hai
static bool
lsp_is_vif(const struct nbrec_logical_switch_port *nbsp)
{
    return !strcmp(nbsp->type, "");
}
//--- Hai
static bool
                      ...
                      struct sset *active_ha_chassis_grps)
{
    ...
    if (op->nbrp) {
        ...
        if (!lsp_is_router(op->nbsp)) {
            //+++ Hai
            if (lsp_is_vif(op->nbsp)) {
                size_t n_nb = op->nbsp->n_queue_source_ports;
                size_t n_sb = op->sb->n_queue_source_ports;
                size_t n_queue = op->sb->n_queue_ids;
                if ((n_nb == op->nbsp->n_queue_min_value) && 0 != n_nb) { // Queues are set in nb
                    // Temp array to sync queue ids between nb and sb
                    int64_t *queue_ids = xmalloc(sizeof(int64_t)*n_nb); 

                    if (n_nb != n_sb) { // This case happen when we add/del queue from nbctl
                        // Copy recent queue ids from sb
                        int i;
                        for (i = 0; i < n_queue; i++) {
                            queue_ids[i] = op->sb->queue_ids[i];    
                        }
                        
                        if (n_nb > n_queue) {
                            // Allocate more id for queue required from nb
                            for (i = n_sb; i <= n_nb; i++) {
                                ovs_assert(queue_id_bitmap);
                                queue_ids[i] = allocate_queueid(queue_id_bitmap);
                            }
                        } else if (n_nb < n_queue) {
                            // Release ids which is del from nb
                            for (i = n_sb; i <= n_nb; i++) {
                                ovs_assert(queue_id_bitmap);
                                bitmap_set0(queue_id_bitmap, queue_ids[i]);
                            }
                        }

                        // Set new value for queue related columns in sb
                        sbrec_port_binding_set_queue_source_ports(op->sb,
                                                                 (const char **)op->nbsp->queue_source_ports, 
                                                                   op->nbsp->n_queue_source_ports);
                        sbrec_port_binding_set_queue_min_value(op->sb,
                                                              (const char **)op->nbsp->queue_min_value, 
                                                              op->nbsp->n_queue_source_ports);
                        sbrec_port_binding_set_queue_ids(op->sb,
                                ...
                                struct ds *actions, struct ds *match)
{
    ...
    ds_clear(match);
    ds_clear(actions);
    if (op->sb->n_queue_ids > 0) {
        for (int i = 0; i < op->sb->n_queue_ids; i++) {
            struct ds json_key = DS_EMPTY_INITIALIZER;
            json_string_escape(op->sb->queue_source_ports[i], &json_key);
            ds_put_format(actions, "set_queue(%ld); output;", op->sb->queue_ids[i]);
            ds_put_format(match, "outport == %s && inport == %s", op->json_key,ds_steal_cstr(&json_key));
...
vif:
        ...
        ds_clear(match);
        if (lsp_is_localnet(op->nbsp) || lsp_is_vif(op->nbsp)) {
